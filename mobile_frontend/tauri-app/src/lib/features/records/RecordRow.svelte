<script lang="ts">
  ...
  let { tx, onSelect }: { tx: Transaction; onSelect?: (t: Transaction) => void } = $props()
...
<button class="row" onclick={() => onSelect?.(tx)}>
  <div class="icon" class:income={tx.flow === 'income'} class:expense={tx.flow === 'expense'}>
    ...
    {:else if tx.categoryIcon === 'investment'}
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
<style>
  ...
  .merchant { font-family: 'Manrope', sans-serif; font-size: 14px; font-weight: 600; color: #1a1c1e; }
  :global(.dark) .merchant { color: #f0f0f3; }
  ...
  .bottom { display: flex; align-items: center; gap: 4px; margin-top: 2px; }
  .category { font-family: 'Public Sans', sans-serif; font-size: 12px; font-weight: 500; color: #6b7b72; }
